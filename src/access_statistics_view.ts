...
class AccessStatsView {
    ...
    private statistics?: _AccessStatistics;
    ...
    public loadAccessStats(changeEvent: JQuery.TriggeredEvent): void {
        ...
        fileReader.onload = (e) => {
            ...
            if (result) {
                ...
                this.statistics = JSON.parse(
                    packedResult[0]
                ) as _AccessStatistics | undefined;
                if (this.statistics)
                    buildTreeView(this.statistics);
                else
...
function formatFunc(
    context: Context, constReadIdx: number, constWriteIdx: number,
    linReadIdx: number, linWriteIdx: number, strideReadIdx: number,
    strideWriteIdx: number, indirReadIdx: number, indirWriteIdx: number
): string {
    const ds = context.chart.data.datasets;
    const dsIdx = context.datasetIndex;
    const dIdx = context.dataIndex;
    if ((dIdx === 0 && (dsIdx === constReadIdx || dsIdx === constWriteIdx)) ||
        (dIdx === 1 && (dsIdx === linReadIdx || dsIdx === linWriteIdx)) ||
        (dIdx === 2 && (dsIdx === strideReadIdx || dsIdx === strideWriteIdx)) ||
        (dIdx === 3 && (dsIdx === indirReadIdx || dsIdx === indirWriteIdx)))
        return ds[dsIdx].stack ?? '';
    return '';
}

function buildDataContainerHist(
    data: _DataContainerAccessStats, root: JQuery,
    collapse: JQuery, expanded: boolean
): void {
                ...
                options: {
                    ...
                    plugins: {
                        ...
                        datalabels: {
                            ...
                            formatter: (_, context) => {
                                return formatFunc(
                                    context, constReadIdx,
                                    constWriteIdx, linReadIdx, linWriteIdx,
                                    strideReadIdx, strideWriteIdx, indirReadIdx,
                                    indirWriteIdx
                                );
                            },
                        },
...
function buildSDFGTreeView(
    data: _SDFGAccessStats, root: JQuery,
    collapse: JQuery, expanded: boolean
): void {
    ...
    switch (sortingCrit) {
        ...
        case 'constant_accesses':
            data.accesses.sort((a, b) => {
                const nA = countAllContainerAccessesOfType(a, sortingCrit);
                const nB = countAllContainerAccessesOfType(b, sortingCrit);
...
function buildSDFGHist(
    data: _AccessStatistics, root: JQuery
): void {
    const getCatReads = (cat: _AccessType) => {
        const reads: Record<string, number> = {};
        const writes: Record<string, number> = {};
                ...
                plugins: {
                    ...
                    datalabels: {
                        ...
                        formatter: function (_, context) {
                            return formatFunc(
                                context, constReadIdx, constWriteIdx,
                                linReadIdx, linWriteIdx, strideReadIdx,
                                strideWriteIdx, indirReadIdx, indirWriteIdx
                            );
                        },
                    },
                },
        ...
        switch (sortingCrit) {
            ...
            case 'constant_accesses':
                data.all_cfg_stats.sort((a, b) => {
                    const nA = countAllSDFGAccessesOfType(a, sortingCrit);
                    const nB = countAllSDFGAccessesOfType(b, sortingCrit);
                    if (sortingStyle === 'descending')
                        return nB - nA;
                    else
                        return nA - nB;
