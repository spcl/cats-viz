...
import { AccessSubset, accessSubsetToString } from '../../utils/display';
...
export abstract class CFVElement extends HTMLCanvasRenderable {
    ...
    public constructor(
        ...
        super(renderer, ctx, renderer.minimapCtx, -1, data);
    }
    ...
    protected _internalDraw(mousepos?: Point2D): void {
        ...
        if (this.hovered && mousepos) {
            if (this.includeSubset) {
                ...
                const subs = memletAttrs?.subset as AccessSubset | undefined;
                if (subs) {
                    const subsetString = accessSubsetToString(subs);
